...
-(int)calcTextStartPosition:(ALIGN)align font_height:(int)font_height enlargeX:(int)enlargeX text:(NSString*)text
{
    if (align == LEFT)
    return 0;
...
{
    int x = [self calcTextStartPosition:align font_height:fontHeight enlargeX:enlargeX text:text];
    return [self drawOut:x y:y text:text fontHeight:fontHeight bold:bold reverse:reverse underLien:underLine deleteLine:deleteLine enlargeX:enlargeX enlargeY:enlargeY rotateAngle:rotateAngle];
}
@end
